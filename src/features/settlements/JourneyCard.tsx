...
import { FlowDots, SettleButton } from '@/components/experience'
...
function Person({ name, relation }: { name: string; relation: 'pays' | 'receives' }) {
  return (
    <div className="flex flex-col items-center gap-1.5">
      <Avatar name={name} size="md" />
      <div>
        <p className="text-sm font-semibold leading-tight text-ink">{firstName(name)}</p>
        <p className="text-[0.6875rem] uppercase tracking-wide text-muted">{relation}</p>
      </div>
    </div>
  )
}
  ...
  return (
    <Card padding="lg" className="relative overflow-hidden text-center">
      ...
      <div className="relative flex flex-col items-center">
        ...
        <Person name={settlement.to} relation="receives" />
